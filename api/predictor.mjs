...
import { tensor2d, sequential, layers, train } from '@tensorflow/tfjs-node';
...
async function initModel() {
  ...
  const xs = tensor2d(inputs);         // [500, 2]
  const ys = tensor2d(labels, [500, 1]); // [500, 1]

  // === Simple neural network ===
  model = sequential();
  model.add(layers.dense({ units: 8, activation: 'relu', inputShape: [2] }));
...
async function predict(grade, progress) {
  const mdl = await initModel();
  const input = tensor2d([[grade, progress]]);
  const pred = mdl.predict(input);
  const prob = (await pred.data())[0];
  return prob;
}
